...
def main():
    if len(sys.argv) < 2:
        print("사용법: python springboot_analysis_tool.py <프로젝트_디렉토리>")
        sys.exit(1)
    ...
    print("Spring Boot 프로젝트 분석을 시작합니다...\n")
...
def main():
    if len(sys.argv) < 2:
        print("사용법: python analysis_tool.py <분석할_파일경로>")
        sys.exit(1)
...
def main():
    if len(sys.argv) < 2:
        print("사용법: python code_checker.py <파일경로>")
        sys.exit(1)
    
    file_path = sys.argv[1]
    print("Python 코드 검사를 시작합니다...")
...
def main():
    parser = argparse.ArgumentParser(description="Spring Boot 검사툴 (actuator 엔드포인트 검사)")
    parser.add_argument("base_url", help="Spring Boot 서버 URL (예: http://localhost:8080)")
    args = parser.parse_args()

    # 검사할 actuator 엔드포인트 목록
    endpoints = {
        "Health": "/actuator/health",
        "Info": "/actuator/info"
    }
    
    print("Spring Boot 검사 시작...")
    for name, endpoint in endpoints.items():
        # 기본 URL과 엔드포인트를 합쳐서 전체 URL 생성
        full_url = args.base_url.rstrip("/") + endpoint
